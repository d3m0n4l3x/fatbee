#!/usr/bin/perl -w
#Correlation Analysis Engine v2 for Fatbee
...
while(1){
	...
	undef(@attackers_ip_addresses);
	foreach $each_log (@log_content){
		if($each_log=~/(.*),(.*),(.*),(.*),(.*),(.*),(.*)/){
			push(@attackers_ip_addresses, $4);
		}
	}
	...
	#abc gih def ghi
	%hash_attackers_ip_addresses=map {$_=>1} @attackers_ip_addresses;
	@unique_attackers_ip_addresses=keys %hash_attackers_ip_addresses;
	...
	foreach $an_unique_attackers_ip_address (@unique_attackers_ip_addresses){
		$counter_a=0;
		foreach $a_attacker_ip_address (@attackers_ip_addresses){
